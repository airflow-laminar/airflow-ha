    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _check_end_conditions(
            task_id=kwargs.get("task_id"), runtime=self._runtime, endtime=self._endtime, maxretrigger=self._maxretrigger, **kwargs
        ):
            ...
            dag_original_start_date = kwargs["dag_run"].conf.get(f"{task_id}-startdate")
            ...
            if not force_run_conf and not force_run_param:
                ...
                elapsed_time = (datetime.now(tz=UTC) - dag_original_start_date).total_seconds()
                _log.info(
                    f"airflow-ha configuration -- runtime: {runtime}, dag_original_start_date: {dag_original_start_date}, elapsed_time: {elapsed_time}"
                )
                ...
                if endtime is not None:
                    ...
                    endtime_as_datetime = datetime.combine(
                        date=dag_original_start_date.astimezone(dag_timezone).date(),
                        time=endtime,
                        tzinfo=dag_timezone,
                    ).astimezone(UTC)
                    _log.info(
