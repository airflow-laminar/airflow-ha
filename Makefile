...
lint:  ## run python linter with ruff
	python -m ruff check airflow_ha
	python -m ruff format --check airflow_ha

# Alias
lints: lint

fix:  ## fix python formatting with ruff
	python -m ruff check --fix airflow_ha
	python -m ruff format airflow_ha
...
test:  ## run python tests
	python -m pytest -v airflow_ha/tests --junitxml=junit.xml

coverage:  ## run tests and collect test coverage
	python -m pytest -v airflow_ha/tests --junitxml=junit.xml --cov=airflow_ha --cov-branch --cov-fail-under=60 --cov-report term-missing --cov-report xml
